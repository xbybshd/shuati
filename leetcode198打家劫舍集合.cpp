...
public:
    ...
    int scan(int l,int r,vector<int>& nums){
        int len=r-l;//分项长度
        if(len==0) return nums[l];//分项长度为空
        dp[l]=nums[l];
        if(len==1) return max(nums[0],nums[1]);//分项长度为1
        for(int i=l;i<=r;i++){
            if(i==l) dp[i]=nums[i];//分项长度大于2是,第1项和第2项要单独考虑
            else if(i==l+1) dp[i]=max(nums[i],nums[i-1]);
            else
            dp[i]=max(dp[i-1],dp[i-2]+nums[i]);
        }
        return dp[r];
    ...
    int rob(vector<int>& nums) {
        if(nums.empty()) return 0;//总体为空
        int l1=nums.size();
        if(l1==1) return nums[0];//总体为1
