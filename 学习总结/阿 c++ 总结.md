...
### static dynamic  
static 可以隐式类型转换，转换void指针，可以无限制转换基类子类，不需要多态
dynamic 转换基类子类必须要多态
...
### delete数组时会报错  
一般都是分配内存小了，在赋值的时候越界了，赋值了大于分配内存的位置，在释放的时候会报错。除了分配你需要的内存空间外还会分配一些管理单元，就在你分配空间的上界和下界，比如内存边界标志(0xFDFDFDFD)，如果是数组在上界会记录数组元素的个数，你要是修改了这些管理信息，delete的时候就会报错。


### 左值引用和const左值引用  
一般的左值引用，无法在作为形参的时候绑定到右值，所以参数类型为了安全，都声明为const左值引用，但是此时带来一个问题，我们想改const左值引用里面的东西怎么办。
